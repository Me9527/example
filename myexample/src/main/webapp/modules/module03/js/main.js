...
function editTable(){
   ...
   var editor = new $.fn.dataTable.Editor( {
    ...
    // Activate an inline edit on click of a table cell
    $('#myexample').on( 'click', 'tbody td:not(:first-child)', function (e) {
        editor.inline( this, {
            onBlur: 'submit'
        } );
    } );
	...
	$.ajax({
       ...
       success: function(data){
    	   if(data.success){
    		  ...
    		  $(location).attr('href', '/myexample/modules/user/jsp/login.jsp');
    	   }
       },
       error : function(err) {
    	   alert(err);
...
function ajaxGetCsrfToken(){
	var url = '/myexample/modules/user/jsp/getCsrf.jsp';
	var token = null;
	$.ajax({
  	   type: "GET",
  	   async :false,
...
function ajaxLogin(){
  ...
  var url = '/myexample/modules/user/login';
  $.ajax({
